...
void eval(char *cmdline)
{
    char *argv[MAXARGS]; // an arrary of pointers
    char buf[MAXLINE];
    strcpy(buf, cmdline);
    ...
    if (!builtin_cmd(argv))
	{
		...
		if (pid == 0)
		{
            // set child process group different than root
            setpgid(0, 0);
            ...
            int ret = execve(argv[0], argv, newenviron); // execve only return if error
            if (ret < 0)
            {
...
int parseline(const char *cmdline, char **argv)
{
    ...
    if ((bg = (*argv[argc-1] == '&')) != 0) {
	    argv[--argc] = NULL;  // replace & with NULL
    }
    return bg;
}
...
void do_bgfg(char **argv)
{
    if (strcmp(*argv, "bg") == 0)
    {
        ...
        {            
            int jid = 0;
            sscanf(param + 1, "%d", &jid);
            //printf("Executing builtin_cmd: bg, jid is:%d.\n", jid);  
...
void sigchld_handler(int sig)
{
    int pid = 0;
    int status = 0;
    // IF we have multiple zombies, do we process them one by one here? YES
    // 而WUNTRACED参数，如果子进程进入暂停执行则马上返回，但终止状态不予理睬。
    while ((pid = waitpid(-1, &status, WNOHANG | WUNTRACED)) > 0) // WNOHANG does the non-blocking
    {
...
void sigint_handler(int sig)
{
    ...
    if (fgPid != 0)
    {
        ...
        //printf("Jobs [%d] (%d) terminated by signal 2\n", job->jid, job->pid);
...
void sigtstp_handler(int sig)
{
    ...
    {
        struct job_t * job = getjobpid(jobs, fgPid);        
        //printf("Jobs [%d] (%d) stopped by signal 20\n", job->jid, job->pid);
        int id = job->pid;
